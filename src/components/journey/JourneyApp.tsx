...
export function JourneyApp({
  ...
  const loadEventAsync = useEventStore((s) => s.loadEventAsync);
  ...
  const handleEmailVerified = useCallback(async (data: {
    sessionId: string;
    email: string;
    eventKey: string;
  ...
  }, [pendingToken, loadEventAsync, track]);
      ...
      <main className="min-h-dvh gradient-hero relative">
        ...
        <div className="flex items-center justify-center min-h-dvh px-4 py-16">
          <div className="w-full max-w-sm space-y-4">
            <EmailGate onVerified={handleEmailVerified} djName={entryDjName || undefined} />
  ...
  const renderStage = () => {
    ...
    switch (currentStage) {
      case 0: return <EventSetup initialDjSlug={initialDjSlug} initialDjName={entryDjName} />;
