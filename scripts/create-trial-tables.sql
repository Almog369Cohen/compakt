-- Trial Management System Migration (Fixed)
-- Based on 016_trial_management.sql with type casting fixes
...
CREATE TABLE IF NOT EXISTS trial_periods (
  ...
  max_events_allowed INTEGER DEFAULT 5,
...
-- Trial periods policies (FIXED TYPE CASTING)
DROP POLICY IF EXISTS "Users can view their own trials" ON trial_periods;
...
DROP POLICY IF EXISTS "Staff can view all trials" ON trial_periods;
...
DROP POLICY IF EXISTS "System can manage trials" ON trial_periods;
CREATE POLICY "System can manage trials" ON trial_periods
  FOR ALL USING (
    current_setting('request.jwt.claims', true)::jsonb->>'role' = 'service_role'
  );

-- Trial events policies (FIXED TYPE CASTING)
DROP POLICY IF EXISTS "Users can view their own trial events" ON trial_events;
...
DROP POLICY IF EXISTS "Staff can view all trial events" ON trial_events;
...
DROP POLICY IF EXISTS "System can manage reminders" ON trial_reminders;
CREATE POLICY "System can manage reminders" ON trial_reminders
  FOR ALL USING (
    current_setting('request.jwt.claims', true)::jsonb->>'role' = 'service_role'
  );

-- Trial settings policies
DROP POLICY IF EXISTS "Anyone can view trial settings" ON trial_settings;
CREATE POLICY "Anyone can view trial settings" ON trial_settings
  FOR SELECT USING (true);

DROP POLICY IF EXISTS "Staff can update trial settings" ON trial_settings;
...
-- Triggers
DROP TRIGGER IF EXISTS update_trial_periods_updated_at ON trial_periods;
